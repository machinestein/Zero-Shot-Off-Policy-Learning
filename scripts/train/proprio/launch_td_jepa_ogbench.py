...
from metamotivo.misc.launcher_utils import all_combinations_of_nested_dicts_for_sweep, flatten, launch_trials, setup_wandb_config
    ...
    "agent": {
        ...
        "model": {
            ...
            "archi": {
                ...
                "psi_predictor": {"name": "ForwardArchi", "hidden_dim": 512, "hidden_layers": 2},
                "actor": {
...
def sweep_antmaze():
    conf = {
        ...
        "agent.train.psi_ortho_coef": [0.1, 1],
        "agent.train.lr_psi": [1.0e-4, 1.0e-5],
    }
    return conf
...
def sweep_cube():
    conf = {
        ...
        "agent.train.psi_ortho_coef": [1, 10],
        "agent.train.lr_psi": [1.0e-4, 1.0e-5],
    }
    return conf
...
if __name__ == "__main__":
    ...
    main(args)
    # uv run -m scripts.train.proprio.launch_td_jepa_ogbench --use_wandb --wandb_gname td_jepa_antmaze_proprio --data_path datasets --workdir_root results --sweep_config sweep_antmaze
    # uv run -m scripts.train.proprio.launch_td_jepa_ogbench --use_wandb --wandb_gname td_jepa_cube_proprio --data_path datasets --workdir_root results --sweep_config sweep_cube
