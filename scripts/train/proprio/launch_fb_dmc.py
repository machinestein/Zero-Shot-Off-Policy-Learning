...
from metamotivo.misc.launcher_utils import all_combinations_of_nested_dicts_for_sweep,\
    flatten, launch_trials, setup_wandb_config
...
def sweep_quadruped():
    conf = {
        ...
        "agent": {
            "model": {"archi": {"z_dim": [50]}},
            "train": {"lr_b": [1e-4, 1e-5], "ortho_coef": [1]},
        },
    }
    return conf
...
def sweep_pointmass():
    conf = {
        ...
        "agent": {
            "model": {"archi": {"z_dim": [50]}},
            "train": {"lr_b": [1e-4], "ortho_coef": [1], "discount": [0.99], "lr_actor": [1e-6]},
        },
    }
    return conf
